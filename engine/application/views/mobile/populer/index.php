...
<script type="text/javascript">
    var News = {
        ...
        loadNews: function (){
            ...
            //load from service
            $.getJSON("<?php echo site_url('service/article/populer'); ?>",{limit:_this.dataLimit,page:_this.page}, function(data){
                ...
                if (data.length > 0){
                    for (var i in data){
                        ...
                        s+= '<div class="media-body">';
                            s+= '<h4 class="media-heading">';
                                s+='<small>'+data[i].category+'</small><br>';
                                s+='<a href="'+data[i].link_href+'">'+data[i].title+'</a>';
                            s+='</h4>';
